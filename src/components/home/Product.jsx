...
export default function ProductSection() {
  ...
  const openProductDetails = (product) => {
    ...
    // In a real app, you would use router.push(`/product/${product.id}`)
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#f8f5f2] to-white">
        <div className="container mx-auto px-4 py-8">
          ...
          <button 
            ...
            className="flex items-center gap-2 text-[#8B4513] hover:text-[#6e3911] mb-8 transition-colors"
          >
            <FaArrowLeft size={20} />
            Back to Products
          </button>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="relative">
              ...
              >
                <FaHeart  size={20} fill={favorites.includes(selectedProduct.id) ? 'currentColor' : 'none'} />
              </button>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 px-6 rounded-lg font-semibold transition-colors ${
                      selectedProduct.inStock
                        ? 'bg-[#8B4513] text-white hover:bg-[#6e3911]'
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-gray-200 pt-6">
                <div className="flex items-center gap-2 text-sm">
                  <FaMapPin  size={16} className="text-[#8B4513]" />
                  <span className="font-medium">Origin:</span>
                  <span className="text-gray-600">{selectedProduct.origin}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <GiCoffeeCup size={16} className="text-[#8B4513]" />
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedProduct.notes.map((note) => (
                    <span 
                      key={note}
                      className="px-3 py-1 bg-[#8B4513]/10 text-[#8B4513] rounded-full text-sm font-medium"
                    >
                      {note}
                    </span>
                  ))}
          ...
          {products.map((product) => (
            ...
            >
              <div className="relative h-56 bg-gray-100 overflow-hidden">
                ...
                >
                  <FaHeart size={16} fill={favorites.includes(product.id) ? 'currentColor' : 'none'} />
                </button>
              ...
              <div className="p-6 text-center">
                <h3 
                  className="text-lg font-semibold text-gray-800 mb-2 leading-snug cursor-pointer hover:text-[#8B4513] transition-colors"
                  onClick={() => openProductDetails(product)}
                >
                  {product.name}
                </h3>
